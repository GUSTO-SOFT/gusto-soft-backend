...
export class MenuService {
  ...
  async create(dto: CreateProductoDto) {
    const duplicated = await this.productosRepo.findOne({
    ...
    const ingredienteIds = ingredientesDto.map(
      (ingrediente) => ingrediente.ingrediente_id,
    );

    const ingredientes =
      await this.inventarioService.findByIds(
        ingredienteIds,
      );
    ...
    if (
      !ingredientes.length ||
      ingredientes.length !==
        new Set(ingredienteIds).size
    ) {
      throw new UnprocessableEntityException(
        errorBody(
          'INGREDIENTES_INVALIDOS',
          'Debes agregar al menos un ingrediente valido',
        ),
      );
    }
  ...
  toResponse(producto: Producto) {
    return {
      ...
      ingredientes:
        this.toIngredientsResponse(producto),
  ...
  private toIngredientsResponse(
    producto: Producto,
  ) {
    if (producto.recipeIngredients?.length) {
      return producto.recipeIngredients.map(
        (recipe) => ({
